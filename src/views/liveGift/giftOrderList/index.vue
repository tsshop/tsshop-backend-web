<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <!-- <el-form-item label="礼物id" prop="liveGiftId">
        <el-input
          v-model="queryParams.liveGiftId"
          placeholder="请输入礼物id"
          clearable
          @keyup.enter.native="handleQuery"
        />
      ...
      <el-form-item label="" prop="payType">
        <el-select v-model="queryParams.payType" placeholder="请选择支付方式">
          ...
          <el-option v-for="item in payTypeOptions" :key="item.key" :label="item.value" :value="item.key"></el-option>
        </el-select>
     </el-form-item>
     <el-form-item label="">
      <el-date-picker
        ...
        type="daterange"
        range-separator="-"
        start-placeholder="支付时间始"
        end-placeholder="支付时间末"
      ></el-date-picker>
    </el-form-item>
      <el-form-item label="">
        <el-date-picker
    ...
    <el-table v-loading="loading" :data="giftList" >
      ...
      <el-table-column label="礼物id" align="center" prop="liveGiftId" />
      <el-table-column label="礼物名称" align="center" prop="liveGiftName" />
      <el-table-column label="礼物数量" align="center" prop="liveGiftNum" />
      <el-table-column label="支付金额" align="center" prop="payAmt" />
      <el-table-column label="支付方式" align="center" prop="payType" :formatter="getPayTypeValue"/>
      ...
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="礼物费率" prop="giftRate">
          <div style="display:flex;align-items: center;">
            <el-input v-model="form.giftRate" style="width:120px;" placeholder="请输入礼物费率" />
            <div style="margin-right:10px;margin-left:5px;">%</div>
            <i class="el-icon-warning-outline" style="font-size:18px;color:#fea585;"></i>
...
<script>
import { getGiftOrderList, getPayTypeMap } from "@/api/liveGift/giftOrderList";
...
export default {
  ...
  created() {
    this.getPayTypeMapOptions()
    this.getList();
    
  },
  methods: {
    getPayTypeValue(row) {
      return this.payTypeOptions.find(it =>it.key === row.payType).value || ''
    },
    getPayTypeMapOptions() {
      let options = []
      getPayTypeMap().then(res => {
        let obj = res.data
        for(let k in obj) {
          options.push({
            key:k,
        ...
        this.payTypeOptions = options
        console.log('this.payType',this.payTypeOptions);
      })
    },
    getValue(row) {
      return row.giftRate + '%'
    ...
    getList() {
      ...
      if (null != this.daterangePayTime && '' != this.daterangePayTime) {
        this.queryParams["payStartTime"] = this.daterangePayTime[0];
        this.queryParams["payEndTime"] = this.daterangePayTime[1];
      }
      getGiftOrderList(this.queryParams).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除直播礼物编号为"' + ids + '"的数据项？').then(function() {
        return delGift(ids);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('liveGift/gift/export', {
        ...this.queryParams
