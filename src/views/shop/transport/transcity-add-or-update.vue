  ...
  methods: {
    ...
    disabledNodes(cityList, allSelectCityList) {
      for (let i = 0; i < this.menuList.length; i++) {
        ...
        for (let j = 0; j < childrens.length; j++) {
          ...
          let allHasCity =
            allSelectCityList.findIndex((item) => city.areaId === item.areaId) > -1;
          let listHasCity =
            cityList.findIndex((item) => city.areaId === item.areaId) > -1;
    ...
    dataFormSubmit() {
      this.$emit(
        "refreshDataList",
        this.rowIndex,
        this.$refs.menuListTree.getCheckedNodes(true),
        this.type
      );
      this.visible = false;
    },
