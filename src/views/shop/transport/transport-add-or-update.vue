  ...
  methods: {
    init(id) {
      ...
      if (this.dataForm.transportId) {
        this.request({
          // 获取运费模板数据
          url: `/shop/transport/info/${this.dataForm.transportId}`,
          method: "get",
        }).then((res) => {
          if (res.isFreeFee) {
            res.transfees[0].status = 0;
          } else {
            res.transfees[0].status = 1;
          }
          this.dataForm = res;
          this.dataForm.hasFreeCondition = !!res.hasFreeCondition;
        });
      }
    },
    getDataList(row, cityList, type) {
      console.log('cityList',row,cityList,type);
