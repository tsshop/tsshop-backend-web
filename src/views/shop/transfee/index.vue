    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['shop:transfee:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['shop:transfee:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="transfeeList" @selection-change="handleSelectionChange">
...
<script>
import { listTransfee, getTransfee, delTransfee, addTransfee, updateTransfee } from "@/api/shop/transfee";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listTransfee(this.queryParams).then(response => {
        this.transfeeList = response.rows;
    ...
    handleUpdate(row) {
      ...
      getTransfee(transfeeId).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.transfeeId != null) {
            updateTransfee(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addTransfee(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除运费项编号为"' + transfeeIds + '"的数据项？').then(function() {
        return delTransfee(transfeeIds);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('shop/transfee/export', {
        ...this.queryParams
      }, `transfee_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
