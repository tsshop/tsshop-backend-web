<template>
  <div class="mod-prod-sku-tag">
    ...
    <el-form-item>
      <el-button size="mini" type="primary" @click="addTag()" v-show="isShowTagInput"
        >确定</el-button
      >
...
export default {
  ...
  methods: {
    init(skuList) {
      ...
      for (let i = 0; i < skuList.length; i++) {
        ...
        for (let j in propertiesArray) {
          ...
          let tagItemNameIndex = skuTags[j].tagItems.findIndex(
            (tagItemName) => tagItemName.propValue === properties[1]
          );
    ...
    handleTagClick() {
      ...
      let specsIndex = this.dbTags.findIndex(
        (spec) => spec.propName === this.addTagInput.propName
      );
      // 如果不是，则说明为用户随便输入
      ...
      //     `/prod/spec/listSpecValue/${this.dbTags[specsIndex].propId}`
      //   ),
      //   method: "get",
      //   params: this.$http.adornParams(),
      // }).then(({ data }) => {
